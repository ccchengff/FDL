...
def wdl_avazu_guest_bottom(guest_input, device="/gpu:0", num_buckets=1000000, embedding_size=128):
...
def wdl_avazu_host_bottom(host_input, device="/gpu:0", num_buckets=1000000, embedding_size=128):
...
def wdl_avazu_top_model(labels, guest_output, host_output, device="/gpu:0"):
    with tf.variable_scope("top", dtype=tf.float32, reuse=tf.AUTO_REUSE, 
        ...
        with tf.device(device):
            ...
            loss = tf.reduce_mean(tf.nn.weighted_cross_entropy_with_logits(
                logits=logits, labels=labels, pos_weight=4.88))
